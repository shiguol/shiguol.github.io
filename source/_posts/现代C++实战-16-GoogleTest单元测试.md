...
C++ 编译器能抓住类型错误，却抓不住「`divide(10, 0)` 该不该抛异常」「阶乘边界算对没有」——这些只能靠**跑起来验证**。[第 15 篇](/2026/07/16/现代C++实战-15-现代设计模式/) 的工厂、策略可以测；本篇用 **GoogleTest** 把测试写进工程：**TEST、Fixture、参数化、Mock、CMake 集成**。

demo：`ref/cpp_demo/basics/testing_demo/`。

<!-- more -->

> 这是「现代 C++ 实战」系列的第 16 篇。建议先读 [第 15 篇：现代设计模式](/2026/07/16/现代C++实战-15-现代设计模式/)。
...
重构 [第 14 篇](/2026/07/15/现代C++实战-14-线程池与背压控制/) 线程池、[第 08 篇](/2026/07/09/现代C++实战-08-错误处理策略/) 错误处理时，**回归测试**是安全网。GoogleTest（gtest）+ GoogleMock（gmock）是 C++ 生态事实标准。
...
原则：**测逻辑不测基础设施**；能注入就不 `new` 真实依赖。与 [第 15 篇](/2026/07/16/现代C++实战-15-现代设计模式/) Strategy / Factory 天然配合。
...
| 篇号 | 标题 | 状态 |
|------|------|------|
| 15 | [现代设计模式](/2026/07/16/现代C++实战-15-现代设计模式/) | ✅ |
