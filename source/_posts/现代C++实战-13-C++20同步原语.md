...
[第 12 篇](/2026/07/13/现代C++实战-12-多线程基础/)用 `thread`、`mutex`、`condition_variable` 搭好了并发地基。C++20 补上最后几块拼图：**`jthread`** 自动 join 与协作式取消、**`semaphore`** 限流、**`latch` / `barrier`** 多线程阶段同步——写法更短、语义更清晰。

这一篇对应 demo：`ref/cpp_demo/concurrency/sync_primitives/`。

<!-- more -->

> 这是「现代 C++ 实战」系列的第 13 篇。建议先读 [第 12 篇：多线程基础](/2026/07/13/现代C++实战-12-多线程基础/)。
...
**协作式取消**：线程必须**主动检查** `stop_requested()`，不能强制杀线程——与 [第 12 篇](/2026/07/13/现代C++实战-12-多线程基础/) 的 mutex 保护一样，是「约定式」安全退出。
...
| 篇号 | 标题 | 状态 |
|------|------|------|
| 12 | [多线程基础](/2026/07/13/现代C++实战-12-多线程基础/) | ✅ |
| **13** | **C++20 同步原语（本篇）** | ✅ |
| 14 | 线程池与背压控制 | 下一篇 |
