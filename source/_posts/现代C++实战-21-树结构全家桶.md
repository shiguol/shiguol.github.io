---
cover: /images/cover/cpp/trees.webp
...
[第 20 篇](/2026/07/03/现代C++实战-20-栈队列与链表/) 的链表是「一对一」串联；树则是**一对多**的分支结构。从二叉搜索树到 AVL、红黑树、B 树、堆——五种经典树形结构，覆盖了内存索引、STL 容器、优先队列和磁盘存储的核心场景。

demo：`ref/cpp_demo/algorithms/tree/`（`binary_tree`、`avl_tree`、`red_black_tree`、`b_tree`、`heap`）。

<!-- more -->

> 这是「现代 C++ 实战」系列的第 21 篇。建议先读 [第 20 篇：栈、队列与链表](/2026/07/03/现代C++实战-20-栈队列与链表/)。
...
层序遍历用 `std::queue<TreeNode*>`，与 [第 20 篇](/2026/07/03/现代C++实战-20-栈队列与链表/) 的队列实现一脉相承。
...
| 大量磁盘数据索引 | B 树 / B+ 树 |
| 算法题、理解原理 | 手写 BST + 遍历 |
| 需要 O(1) 均摊插入均摊查找 | 哈希表（[第 19 篇](/2026/07/02/现代C++实战-19-哈希表实现/)） |
...
下一篇进入**图**：BFS 找最短、DFS 找路径、Dijkstra 算权重——见 [第 22 篇：图算法](/2026/07/05/现代C++实战-22-图算法/)（计划）。
