...
> 这是「大模型数学速成」系列的第 8 篇。建议已读 [第 00–07 篇](/2026/07/04/大模型数学速成（00）：读-Transformer-前需要哪些数学？/)。下一篇 **多头注意力**。
...
本系列约定：**行 = 特征维，列 = 序列位置**（[第 01 篇](/2026/07/05/大模型数学速成（01）：张量、维度与「列-token」/)）。
...
| **Attention 掩码** | 通常无（或 padding 掩码） | **下三角因果掩码**（[第 05 篇](/2026/07/09/大模型数学速成（05）：注意力机制与Softmax/)） |
| **位置编码** | 可学习绝对位置 或 正弦；部分 ViT 无显式 RoPE | 现代 LLM 多用 **RoPE**（[第 07 篇](/2026/07/11/大模型数学速成（07）：RoPE-用旋转编码位置/)），只作用于 Q/K |
| **Norm** | 常见 **LayerNorm**（$\gamma, \beta$） | 常见 **RMS Norm**（仅 $\gamma$）（[第 04 篇](/2026/07/08/大模型数学速成（04）：LayerNorm-RMS-Norm-与残差连接/)） |
| **FFN 激活** | 常 **GELU**，单路上投影 | 常 **SwiGLU** 门控 FFN（[第 06 篇](/2026/07/10/大模型数学速成（06）：前馈网络FFN-GELU与SwiGLU/)） |
...
| 步骤 | ViT | LLM |
|------|-----|-----|
| Q/K/V 投影 | 同 [第 03 篇](/2026/07/07/大模型数学速成（03）：Q-K-V-投影-同一输入三种角色/) | 同 |
| $Q^\top K/\sqrt{d}$ + Softmax | 全连接（无因果） | 掩码后 Softmax |
| 残差 + Norm | 同 [第 04 篇](/2026/07/08/大模型数学速成（04）：LayerNorm-RMS-Norm-与残差连接/) | 同 |
| FFN | 同 [第 06 篇](/2026/07/10/大模型数学速成（06）：前馈网络FFN-GELU与SwiGLU/) | 同（激活常不同） |
...
本系列第 05 篇已区分 Self / Cross-Attention；多模态是 Cross 的典型场景。细节因架构而异（如是否拼接、是否共享权重），但**矩阵形状与握手规则**仍遵循 [第 01–02 篇](/2026/07/05/大模型数学速成（01）：张量、维度与「列-token」/)。
...
| 你想搞懂… | 优先复习 |
|-----------|----------|
| 权重形状、列=token | [01 张量](/2026/07/05/大模型数学速成（01）：张量、维度与「列-token」/)、[02 矩阵乘](/2026/07/06/大模型数学速成（02）：矩阵乘法-神经网络的基本变换/) |
| Attention 公式 | [03 Q/K/V](/2026/07/07/大模型数学速成（03）：Q-K-V-投影-同一输入三种角色/)、[05 Softmax](/2026/07/09/大模型数学速成（05）：注意力机制与Softmax/) |
| 训练稳定性 | [04 Norm+残差](/2026/07/08/大模型数学速成（04）：LayerNorm-RMS-Norm-与残差连接/) |
| LLM FFN | [06 FFN](/2026/07/10/大模型数学速成（06）：前馈网络FFN-GELU与SwiGLU/) |
| 长文本 / 位置 | [07 RoPE](/2026/07/11/大模型数学速成（07）：RoPE-用旋转编码位置/) |
| **ViT vs LLM 总览** | **本篇** |
...
| 篇号 | 标题 | 状态 |
|------|------|------|
| 07 | [RoPE](/2026/07/11/大模型数学速成（07）：RoPE-用旋转编码位置/) | ✅ |
