...
[上一篇](/2026/07/06/大模型数学速成（02）：矩阵乘法-神经网络的基本变换/)我们学会了 $y = W \cdot x + b$ 的「握手规则」。在 Transformer 的注意力层里，**同一份输入特征会连乘三次**——分别得到 Q、K、V。为什么要三套权重？权重 $W_q$ 和激活 Q 有什么区别？

这一篇把 Q/K/V 投影一次讲透，为第 05 篇的注意力与 Softmax 铺路。

<!-- more -->

> 这是「大模型数学速成」系列的第 3 篇。建议先读 [第 02 篇：矩阵乘法](/2026/07/06/大模型数学速成（02）：矩阵乘法-神经网络的基本变换/)。下一篇讲 **LayerNorm、RMS Norm 与残差连接**。
...
典型形状（方阵投影）：$W_q, W_k, W_v$ 均为 `[768, 768]`，$Q, K, V$ 均为 `[768, S]`——列数 S 不变，与 [第 02 篇](/2026/07/06/大模型数学速成（02）：矩阵乘法-神经网络的基本变换/) 一致。
...
| 篇号 | 标题 | 状态 |
|------|------|------|
| 02 | [矩阵乘法](/2026/07/06/大模型数学速成（02）：矩阵乘法-神经网络的基本变换/) | ✅ |
| **03** | **Q/K/V 投影（本篇）** | ✅ |
