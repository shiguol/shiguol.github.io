...
[第 05 篇](/2026/07/09/大模型数学速成（05）：注意力机制与Softmax/)我们用的是「一整块」768 维 Q/K/V 做一次 Attention。实际模型里常见 **12、16、32 个头（head）**——同一层里多组更小的 Attention **并行**运行，再拼回去。为什么要拆头？维度怎么变？

这一篇讲清 **分头、逐头 Attention、concat 与输出投影 $W_o$** 的全流程。

<!-- more -->

> 这是「大模型数学速成」系列的第 9 篇。建议先读 [第 05 篇：Attention 与 Softmax](/2026/07/09/大模型数学速成（05）：注意力机制与Softmax/)、[第 07 篇：RoPE](/2026/07/11/大模型数学速成（07）：RoPE-用旋转编码位置/)。下一篇 **GQA**。
...
本系列约定：矩阵 **[行 = 特征，列 = token]**（[第 01 篇](/2026/07/05/大模型数学速成（01）：张量、维度与「列-token」/)）。
...
**RoPE**（[第 07 篇](/2026/07/11/大模型数学速成（07）：RoPE-用旋转编码位置/)）在分头之后、Attention 之前，**对每个 head 的 Q/K 独立施加**（每头 $d_\text{head}$ 维上的旋转）。
...
- $O$ 形状 `[d, S]`，与 Attention 输入同形，可 **残差相加**（[第 04 篇](/2026/07/08/大模型数学速成（04）：LayerNorm-RMS-Norm-与残差连接/)）
...
[第 08 篇](/2026/07/12/大模型数学速成（08）：ViT层vs-LLM层-概念对照总表/) 对照表：ViT 与 LLM **都用 MHA**（或变体），差别在掩码（因果 vs 双向）、RoPE、Norm 等，**分头 concat 结构相同**。
...
| 篇号 | 标题 | 状态 |
|------|------|------|
| 08 | [ViT vs LLM 对照](/2026/07/12/大模型数学速成（08）：ViT层vs-LLM层-概念对照总表/) | ✅ |
| **09** | **多头注意力（本篇）** | ✅ |
| 10 | GQA：分组查询注意力 | 下一篇 |
