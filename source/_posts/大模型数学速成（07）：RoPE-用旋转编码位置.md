...
> 这是「大模型数学速成」系列的第 7 篇。建议先读 [第 06 篇：FFN](/2026/07/10/大模型数学速成（06）：前馈网络FFN-GELU与SwiGLU/)。下一篇 **ViT 层 vs LLM 层对照总表**。

## 一、顺序问题：Attention 是「置换等变」的

[第 05 篇](/2026/07/09/大模型数学速成（05）：注意力机制与Softmax/) 的注意力只看点积相似度。若把 token 列整体重排（Q、K、V 同步重排），每个 token 关注谁不变——**模型不知道谁在前谁在后**。
...
沿用 [第 01 篇](/2026/07/05/大模型数学速成（01）：张量、维度与「列-token」/) 约定，$Q$ 为 `[d, S]`，第 $i$ 列对应序列位置 $i$（或 batch 内偏移后的绝对位置）。
...
| 篇号 | 标题 | 状态 |
|------|------|------|
| 06 | [FFN / GELU / SwiGLU](/2026/07/10/大模型数学速成（06）：前馈网络FFN-GELU与SwiGLU/) | ✅ |
| **07** | **RoPE：用旋转编码位置（本篇）** | ✅ |
