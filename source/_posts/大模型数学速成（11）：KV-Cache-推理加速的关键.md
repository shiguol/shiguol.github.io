...
没有 **KV Cache**，大模型逐 token 生成会反复重算整段历史的 K、V——复杂度爆炸、延迟不可接受。[第 10 篇](/2026/07/14/大模型数学速成（10）：GQA-分组查询注意力/) 讲了用 GQA 压缩 Cache 体积；本篇收官：**Prefill / Decode 两阶段**、Decode 单步在算什么、显存怎么估，以及多模态里 Cache 怎么共享。

<!-- more -->

> 这是「大模型数学速成」系列的**第 11 篇（收官）**。建议先读 [第 10 篇：GQA](/2026/07/14/大模型数学速成（10）：GQA-分组查询注意力/)。

## 一、O(N²) 重复计算问题

生成第 $t$ 个 token 时，Attention 需要当前 Q 与**位置 $1 \ldots t$ 的全部 K** 做点积（[第 05 篇](/2026/07/09/大模型数学速成（05）：注意力机制与Softmax/)）。
...
多头 / GQA 时，Cache 按 **K/V head** 分块存储；GQA 下 $h_\text{kv} < h_q$，Cache 更小（[第 10 篇](/2026/07/14/大模型数学速成（10）：GQA-分组查询注意力/)）。
...
- **文本 token** 的 K/V
- **图像 patch token** 的 K/V（ViT 编码后并入序列，见 [第 08 篇](/2026/07/12/大模型数学速成（08）：ViT层与LLM层-概念对照总表/)）
...
> 大模型数学速成系列**全部 11 篇完结**。感谢跟读——从「列 = token」到 KV Cache，希望再读 Transformer 论文或 config 时，公式能对上号。

### 全系列导航

| 篇号 | 标题 |
|------|------|
| 00 | [读 Transformer 前需要哪些数学？](/2026/07/04/大模型数学速成（00）：读Transformer前需要哪些数学/) |
| 01 | [张量、维度与「列 = token」](/2026/07/05/大模型数学速成（01）：张量维度与列等于token/) |
| 02 | [矩阵乘法：神经网络的基本变换](/2026/07/06/大模型数学速成（02）：矩阵乘法-神经网络的基本变换/) |
| 03 | [Q/K/V 投影](/2026/07/07/大模型数学速成（03）：QKV投影-同一输入三种角色/) |
| 04 | [LayerNorm、RMS Norm 与残差](/2026/07/08/大模型数学速成（04）：LayerNorm-RMSNorm与残差连接/) |
| 05 | [注意力机制与 Softmax](/2026/07/09/大模型数学速成（05）：注意力机制与Softmax/) |
| 06 | [FFN：GELU 与 SwiGLU](/2026/07/10/大模型数学速成（06）：前馈网络FFN-GELU与SwiGLU/) |
| 07 | [RoPE](/2026/07/11/大模型数学速成（07）：RoPE-用旋转编码位置/) |
| 08 | [ViT 层 vs LLM 层](/2026/07/12/大模型数学速成（08）：ViT层与LLM层-概念对照总表/) |
| 09 | [多头注意力](/2026/07/13/大模型数学速成（09）：多头注意力-多个专家各看各的/) |
| 10 | [GQA](/2026/07/14/大模型数学速成（10）：GQA-分组查询注意力/) |
| **11** | **KV Cache（本篇，收官）** |
