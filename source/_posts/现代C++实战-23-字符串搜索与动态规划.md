---
cover: /images/cover/cpp/string-dp.png
...
[第 22 篇](/2026/07/23/现代C++实战-22-图算法/) 的图算法处理「关系」；本篇进入算法面试的两大支柱：**字符串搜索**（在文本中找模式）和**动态规划**（把大问题拆成重叠子问题）。KMP 和 Boyer-Moore 解决搜索，背包和 LCS 解决 DP——掌握这些，LeetCode 中等题大半有思路。

demo：`ref/cpp_demo/algorithms/string_search/` + `ref/cpp_demo/algorithms/dynamic_programming/`。

<!-- more -->

> 这是「现代 C++ 实战」系列的第 23 篇。建议先读 [第 22 篇：图算法](/2026/07/23/现代C++实战-22-图算法/)。
...
第三季算法篇收官。下一篇进入**第四季进阶项目**：用 cpp-httplib 搭 REST 服务、nlohmann/json 做序列化——见 [第 24 篇：HTTP 服务与 JSON](/2026/07/25/现代C++实战-24-HTTP服务与JSON/)（计划）。
