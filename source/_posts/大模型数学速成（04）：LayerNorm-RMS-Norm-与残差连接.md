...
[上一篇](/2026/07/07/大模型数学速成（03）：Q-K-V-投影-同一输入三种角色/)我们算出 Q、K、V——但在乘 $W_q$ 之前，输入通常先过 **LayerNorm 或 RMS Norm**。算完注意力或 FFN 之后，还要加一条 **残差连接** 把原输入「抄送」回来。

这一篇搞懂：**为什么要归一化、两种 Norm 差在哪、Pre-LN 与 Post-LN 怎么排、残差为什么能训深网络**。

<!-- more -->

> 这是「大模型数学速成」系列的第 4 篇。建议先读 [第 03 篇：Q/K/V 投影](/2026/07/07/大模型数学速成（03）：Q-K-V-投影-同一输入三种角色/)。下一篇讲 **注意力机制与 Softmax**。
...
在本系列约定下（[第 01 篇](/2026/07/05/大模型数学速成（01）：张量、维度与「列-token」/)）：
...
**维度检查**（与 [第 01 篇](/2026/07/05/大模型数学速成（01）：张量、维度与「列-token」/) 对照）：
...
Norm 在 Q/K/V 投影**之前**（Pre-LN），所以 [第 03 篇](/2026/07/07/大模型数学速成（03）：Q-K-V-投影-同一输入三种角色/) 里的 $X$ 通常是 **Norm 之后的特征**。
...
| 篇号 | 标题 | 状态 |
|------|------|------|
| 03 | [Q/K/V 投影](/2026/07/07/大模型数学速成（03）：Q-K-V-投影-同一输入三种角色/) | ✅ |
