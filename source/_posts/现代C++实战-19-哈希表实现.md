...
[第 18 篇](/2026/07/19/现代C++实战-18-排序算法与std-sort/) 用 `std::sort` 做 O(log n) 查找前的准备；**哈希表**把查找压到平均 **O(1)**——键经哈希函数映射到桶，碰撞用链地址或开放寻址解决。理解这两条路，就摸清了 `std::unordered_map` 的底层地图。

demo：`ref/cpp_demo/algorithms/hash_table/`（链地址、线性探测、双重哈希 + 与标准库性能对比）。

<!-- more -->

> 这是「现代 C++ 实战」系列的第 19 篇。建议先读 [第 18 篇：排序算法](/2026/07/19/现代C++实战-18-排序算法与std-sort/)。
...
| 篇号 | 标题 | 状态 |
|------|------|------|
| 18 | [排序算法与 std::sort](/2026/07/19/现代C++实战-18-排序算法与std-sort/) | ✅ |
| **19** | **哈希表实现（本篇）** | ✅ |
| 20 | 栈、队列与链表 | 下一篇 |
