...
这一篇不深入任何单一特性，而是把 **C++03 → C++23** 的演进脉络梳理清楚，并带你跑一遍 `version_features` demo——后面 25 篇特性文章，都可以在这张地图上找到位置。

<!-- more -->

> 这是「现代 C++ 实战」系列的第 2 篇。对应 demo：`ref/cpp_demo/basics/version_features/`。建议先读 [第 01 篇：CMake 与现代构建](/2026/07/02/现代C++实战-01-CMake与现代构建/)。
...
| C++23 | 2023 | 202302L | `-std=c++23` | 持续演进中 |
...
- 本系列 demo 标注了所需标准（C++11～C++23），CMake 里用 `CMAKE_CXX_STANDARD` 或 `set_target_properties` 指定即可（见第 01 篇）。
...
C++23 仍在各编译器的「部分支持」阶段，但几个特性已经值得了解：
...
本系列第 17 篇会专门跑 `cpp23_features` demo。现在知道它们存在即可——**不必追最新，但要清楚方向**。

## 六、编译器怎么选？

不同编译器、不同版本，对标准的支持进度不同。日常开发可以记这张简表：

| 编译器 | 推荐最低版本 | C++17 | C++20 | C++23 | 常见场景 |
|--------|-------------|-------|-------|-------|----------|
| **GCC** | 11+ | ✅ 完整 | ✅ 大部分 | 部分 | Linux 服务器、Docker |
| **Clang** | 14+ | ✅ 完整 | ✅ 大部分 | 部分 | macOS、clangd |
| **Apple Clang** | 14+ (Xcode 14) | ✅ | 大部分 | 少量 | macOS 本地开发 |
| **MSVC** | VS 2019 16.11+ | ✅ | 大部分 | 部分 | Windows |
...
- **C++23** 继续补齐 expected、print 等实用特性
- **`version_features` demo** 按版本拆分，可逐个编译运行
...
| 01 | [CMake 与现代构建](/2026/07/02/现代C++实战-01-CMake与现代构建/) | ✅ |
| **02** | **C++ 版本演进一览（本篇）** | ✅ |
| 03 | 移动语义与右值引用 | 下一篇 |
