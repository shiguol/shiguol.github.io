---
cover: /images/cover/cpp/graph.webp
...
[第 21 篇](/2026/07/04/现代C++实战-21-树结构全家桶/) 的树是一对多分支；**图**则允许任意顶点之间连边——社交网络、路由表、任务依赖、编译单元关系，本质都是图。BFS 找最短、DFS 探路径、Dijkstra 算权重、拓扑排序排依赖——四大经典算法覆盖 90% 的工程场景。

demo：`ref/cpp_demo/algorithms/graph/`（`graph.h`、`bfs`、`dfs`、`dijkstra`、`topological_sort`）。

<!-- more -->

> 这是「现代 C++ 实战」系列的第 22 篇。建议先读 [第 21 篇：树结构全家桶](/2026/07/04/现代C++实战-21-树结构全家桶/)。
...
[第 20 篇](/2026/07/03/现代C++实战-20-栈队列与链表/) 的栈与队列、[第 21 篇](/2026/07/04/现代C++实战-21-树结构全家桶/) 的堆，在这里组合成完整图算法工具箱。
...
下一篇进入**字符串搜索与动态规划**：KMP、Boyer-Moore、背包与 LCS——见 [第 23 篇：字符串搜索与动态规划](/2026/07/06/现代C++实战-23-字符串搜索与动态规划/)（计划）。
